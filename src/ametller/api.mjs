...
export class AmetllerClient {
  ...
  authStatus() {
    const at = tokenStatus(this.session?.access_token);
    return {
      signed_in: Boolean(this.session?.refresh_token || at.valid),
      access_token_valid: at.valid,
  ...
  async #accessToken() {
    if (this.session) {
      ...
      if (this.session.refresh_token) {
        const t = await refreshToken(this.session.refresh_token);
  ...
  // Find the customer's active basket, or create one.
  async #ensureBasket() {
    if (this.basketId) return this.basketId;
    if (this.registered && this.customerId) {
      const data = await this.#call(
        `/customer/shopper-customers/v1/organizations/${ORG}/customers/${this.customerId}/baskets?${qs({})}`,
      );
  ...
  async getCart() {
    const id = await this.#ensureBasket();
  ...
  addManyToCart(items) {
    return this.#ensureBasket().then((id) =>
  ...
  getOrders(page = 1, limit = 10) {
    ...
    return this.#call(
      `/customer/shopper-customers/v1/organizations/${ORG}/customers/${this.customerId}/orders?${qs({ offset, limit })}`,
    );
  }
...
export function compactCart(c) {
  if (!c) return { products_count: 0, total: null, lines: [] };
  return {
    basket_id: c.basketId,
...
export function lineIsAvailable() {
  return true; // SCAPI omits unavailable products from search; refined during reorder testing.
}
